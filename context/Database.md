...
## ⚠️ IMPLEMENTATION NOTE - BaseEntity Convention

Hầu hết các Entity trong hệ thống kế thừa `BaseEntity`, tự động có **4 trường audit** sau (KHÔNG liệt kê lại trong từng bảng):
- **created_at** (Timestamp, Not Null): Thời gian tạo record. Auto-set bởi JPA `@CreatedDate`.
- **updated_at** (Timestamp, Not Null): Thời gian cập nhật cuối. Auto-set bởi JPA `@LastModifiedDate`.
- **created_by** (BigInt, Nullable): User ID người tạo. Auto-set bởi `AuditorAware`.
- **updated_by** (BigInt, Nullable): User ID người cập nhật. Auto-set bởi `AuditorAware`.

**Các bảng KHÔNG extends BaseEntity** (có lý do riêng — ghi rõ trong mô tả từng bảng):
- `audit_logs` — Append-only, chỉ có `created_at`
- `request_histories` — Append-only, chỉ có `created_at`
- `jwt_tokens` — Kỹ thuật, chỉ có `created_at`
- `project_members` — Bảng trung gian, chỉ có `joined_at`
- `request_attachments` — Bảng trung gian, không có timestamp
- `phase_category_budgets` — Bảng trung gian, không có timestamp
- `user_profiles` — Shares PK với `users`, không có audit fields riêng
- `expense_categories` — Quản lý audit fields thủ công (có `is_system_default`)

**Các bảng có @Version (Optimistic Locking):**
- `departments` — `version` (BigInt): Concurrent budget updates
- `projects` — `version` (BigInt): Concurrent budget updates
- `wallets` — `version` (BigInt): Concurrent balance updates
...
### `file_storages`
*Extends `BaseEntity` (có created_at, updated_at, created_by, updated_by).*
...
*Extends `BaseEntity`. Các trường `enabled`, `account_non_expired`, `account_non_locked`, `credentials_non_expired` là implementation detail của Spring Security `UserDetails`.*
* **id** (PK, BigInt, Auto-increment)
...
*Extends `BaseEntity`. Shares PK với `users`.*
* **user_id** (PK, FK): Khóa chính chung với bảng `users`.
...
* **version** (BigInt): Optimistic Locking cho concurrent budget updates.
---
...
*Extends `BaseEntity`. Append-only — KHÔNG UPDATE, KHÔNG DELETE.*
...
### `system_configs`
*Extends `BaseEntity`. PK là `config_key`, không dùng auto-increment id.*
* **key** (Varchar, PK): Tên cấu hình (VD: PIN_MAX_RETRY). Mapping: `config_key` trong SQL.
* **value** (Varchar): Giá trị. Mapping: `config_value` trong SQL.
